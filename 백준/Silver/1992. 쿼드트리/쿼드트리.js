...
function compressVideo(x, y, size) {
  if (size == 1) {
    // [color, 1] 형태로 값을 반환
    return [String(paper[x][y]), 1];
  }
  const halfSize = size / 2;
  const topLeft = compressVideo(x, y, halfSize);
  const topRight = compressVideo(x, y + halfSize, halfSize);
  const bottomLeft = compressVideo(x + halfSize, y, halfSize);
  const bottomRight = compressVideo(x + halfSize, y + halfSize, halfSize);
...
function combineResults(...results) {
  const colors = results.map((res) => res[0]);
  const sumCount = results.reduce((acc, res) => acc + res[1], 0);
